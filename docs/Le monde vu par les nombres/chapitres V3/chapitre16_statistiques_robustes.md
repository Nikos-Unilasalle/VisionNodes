# Chapitre 16 — Se méfier des données : les statistiques robustes

![Un juge impassible écoute une foule de témoins ; l'un hurle des absurdités, mais le juge ne lui accorde pas plus de poids qu'aux autres](../figures/fig_ch16_couverture.jpg)
*Une seule donnée folle peut détruire une mesure juste pour mille autres. Être robuste, c'est décider à l'avance combien de poids une observation a le droit de prendre — et refuser de croire l'aberration.*
...
> *Un capteur fou tire sur la moyenne de tout son poids ; sur la médiane, d'un seul cran*

![fig_ch16_obs1_median](../figures/fig_ch16_obs1_median.svg)
...
En ne lisant que le signe, la médiane jette de l'information utile quand les données sont propres : la moyenne est alors plus *efficace* (de plus faible variance). Et sa fonction d'influence en escalier (`sign`) est discontinue en zéro — un défaut numérique que Huber corrige au §16.3 en arrondissant ce coude.

### Subtilité — nombre pair et égalité de valeurs
...
> *Avant de dire qu'une mesure est « loin », il faut un mètre que l'aberration ne tord pas*
...
> *Jusqu'à quelle distance un résidu a-t-il le droit de tirer sur l'estimation*

![fig_ch16_obs2_mestimators](../figures/fig_ch16_obs2_mestimators.svg)

### L'intention

La médiane est robuste mais grossière (elle ne lit que des signes) ; les moindres carrés sont précis mais fragiles (influence non bornée). On veut un réglage continu entre les deux : précis près de la cible, robuste au loin, et avec un curseur pour décider *à partir d'où* on se méfie.
...
- **Tukey** (biweight) : ψ monte, culmine, puis **redescend à zéro** au-delà de son seuil. Un outlier franc reçoit une influence *strictement nulle* — il disparaît de l'équation.
...
### Réglage — le seuil en unités de bruit, et l'initialisation de Tukey
...
> *Une négociation où l'on baisse le micro de qui parle trop fort, tour après tour*
...
### Subtilité — division par zéro et ré-estimation de l'échelle
...
## 16.5 — RANSAC : la robustesse par consensus

> *Tirer un petit jury au sort, lui demander une version des faits, compter qui la confirme*

![fig_ch16_obs3_ransac](../figures/fig_ch16_obs3_ransac.svg)
...
L'algorithme RANSAC (pour *Random Sample Consensus*) résout le problème par un vote démocratique basé sur le consensus, en procédant ainsi :

1. **Le tirage d'un micro-jury** : Au lieu d'écouter tout le monde, on tire au hasard un groupe minimal de témoins, juste assez pour pouvoir formuler une hypothèse unique. Par exemple, pour tracer une ligne, deux témoins suffisent (car deux points définissent une droite).
2. **La version des faits** : On trace la ligne d'horizon uniquement basée sur les déclarations de ce micro-jury.
3. **Le vote de consensus** : On présente cette ligne d'horizon à tous les autres témoins de la pièce. Chacun vote : « Cette ligne passe-t-elle près de mon observation ? » (à une tolérance t près). Si c'est le cas, le témoin rejoint le consensus et est compté comme *inlier*. Sinon, sa voix est ignorée.
4. **La répétition** : On répète cette opération N fois avec de nouveaux jurys tirés au hasard. Si un jury contient ne serait-ce qu'un seul menteur, sa ligne sera absurde et ne récoltera presque aucun vote. En revanche, le jour où le hasard réunit un jury composé exclusivement de témoins honnêtes, leur ligne passera naturellement près de tous les autres témoins honnêtes de la pièce, récoltant un consensus massif (40 % des voix).
5. **Le verdict final** : On conserve la ligne qui a réuni le plus grand nombre de votes. On écarte définitivement tous les menteurs qui ont voté contre, et on recalcule la ligne finale par la méthode classique sur le groupe entier des citoyens honnêtes ainsi identifiés.
...
On raisonne sur les probabilités. Si w est la proportion d'inliers et n la taille de l'échantillon minimal, la probabilité qu'un tirage soit entièrement pur est wⁿ, donc qu'il soit contaminé 1 − wⁿ, donc que les N tirages soient *tous* contaminés (1 − wⁿ)ᴺ. On veut que cette dernière soit assez faible (au plus 1 − p) :
...
Trois leçons. Doubler n (4 → 8) multiplie N par ~16 (72 → 1177) : d'où la règle d'or « toujours le plus petit échantillon minimal ». Faire chuter w (0,5 → 0,3) multiplie N par ~8 (72 → 567) : la moindre dégradation du taux d'inliers coûte cher. C'est le calcul qu'on pose pour budgéter un recalage d'images ou un *structure-from-motion* (chapitre 8).

### Réglage — le seuil d'inlier et l'échantillon minimal
...
> *RANSAC compte les mains levées ; ses variantes mesurent l'enthousiasme*
...
## Encadré final — être robuste, c'est décider à l'avance ce qu'on refuse de croire
...
|---|---|---|---|
| `fig_ch16_couverture` | chapeau | Illustration : un juge impassible n'accordant pas plus de poids au témoin qui hurle | JPG/PNG |
| `fig_ch16_obs1_median` | 16.1 | Échantillon avec un outlier : la moyenne dérive, la médiane reste plantée | SVG |
| `fig_ch16_obs2_mestimators` | 16.3 | Fonctions d'influence ψ superposées : MCO, médiane, Huber, Tukey | SVG |
| `fig_ch16_obs3_ransac` | 16.5 | Nuage de points avec outliers : un tirage minimal, sa droite, ses inliers | SVG |
| `fig_ch16_01_mad` | 16.2 | MAD vs écart-type : l'aberration explose l'un, laisse l'autre intact | SVG |
| `fig_ch16_02_ransac_N` | 16.5 | Courbe N en fonction de w pour n = 4 et n = 8 : explosion exponentielle | SVG |
