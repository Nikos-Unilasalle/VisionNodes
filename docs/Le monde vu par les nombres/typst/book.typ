...
// Breakable box for print/KDP. The modern theme wraps each admonition in a
// non-breakable 2-column grid (icon | body), so long boxes overflow the page
// instead of splitting. We override the theme's `box` function: bookly's
// info-box / tip-box / warning-box / question-box all delegate to theme.box,
// so injecting it here fixes every box regardless of per-chapter `import *`.
// A single breakable block (no grid, no place(): both block page-breaking),
// with the icon flowing inline at the start.
#let _breakable-box(title: none, icon: "info", color: rgb(29, 144, 208), body) = block(
  breakable: true,
  width: 100%,
  ...
  inset: 1em,
  {
    box(baseline: 0.25em, color-svg("resources/images/icons/" + icon + ".svg", color, width: 1.25em))
    h(0.6em)
    body
  }
)

#let theme1 = modern + (theme: option1-theme-func, box: _breakable-box)
...
#page(header: none, footer: none,
  ...
  #v(0.3em)
  Lien de téléchargement : nikos-unilasalle.github.io/VisionNodes

  #v(0.2em)
  Code source (pour programmer vos propres nœuds) : github.com/Nikos-Unilasalle/VisionNodes
...
#appendix[
  ...
  #include "chapters/annexe_B_cahier_de_code.typ"
]
