# Chapitre 17 — Retrouver un point : descripteurs locaux et appariement

![Deux photos d'une même façade avec des flèches reliant des points-clés homologues](../figures/fig_ch17_couverture.jpg)
*Reconnaître un même point du monde sous un autre éclairage, une autre échelle ou un autre angle : le descripteur local jette le variable pour ne garder que le stable.*
...
## 17.1 — Le problème de l'appariement : pourquoi les pixels bruts ne suffisent pas

> *Reconnaître une structure sans se laisser abuser par sa couleur exacte*
...
- **Missions :**
  ...
  2. Dans le *Python Node*, remplacez ORB par un SSD sur des patchs 16×16 bruts. Observez l'incohérence des lignes.
  3. Augmentez l'angle de rotation (10°, 30°, 60°, 90°). Notez à quel angle ORB commence à perdre l'appariement.

---

## 17.2 — Échelle caractéristique : à quel point zoomer ?

> *Le bon recul de lecture, qui s'ajuste à la taille de la structure*
...
- **Missions :**
  1. Repérez trois points-clés sur des structures de tailles différentes. Vérifiez que le rayon est proportionnel à la structure.
  2. Vérifiez que sur la vue B (agrandie), le cercle d'un point homologue est deux fois plus grand.
  3. Comprenez pourquoi un détecteur sans espace d'échelle (Harris nu) échoue à apparier ces deux images.

---

## 17.3 — HOG : la rose des vents des orientations

> *Un plan de bordures résumé par des roses des orientations*
...
- **Missions :**
  1. Variez la luminosité et le contraste. Constatez la stabilité des glyphes HOG.
  2. Faites tourner l'image de 0° à 45°. Relevez à quel angle le descripteur devient méconnaissable.
  3. Déterminez si HOG est adapté pour trier des pièces jetées en vrac à angles quelconques.

---

## 17.4 — SIFT : le descripteur de référence

> *Orienter la carte locale pour la lire toujours dans le même sens*
...
- **Missions :**
  ...
  2. Comptez le nombre d'appariements valides à 0°, 45° et 90°. La baisse est-elle abrupte ou progressive ?
  3. Comparez avec HOG sous les mêmes rotations.

---

## 17.5 — ORB et BRIEF : la rapidité binaire

> *La comparaison d'intensités, codée en bits*
...
- **Missions :**
  ...
  2. Introduisez un flou ou dégradez la perspective via Warp Perspective. Lequel des deux conserve le plus de correspondances ?
  3. Évaluez si le gain de vitesse d'ORB est indispensable pour un flux live à 30 fps sur votre machine.

---

## 17.6 — Le ratio test de Lowe : rejeter l'ambiguïté

> *Exiger un coup de cœur, éliminer le doute*
...
## 17.7 — RANSAC et homographie : imposer la cohérence géométrique

> *Trouver l'accord général dans un nuage de suspects*
...
- **Missions :**
  ...
  2. Observez l'alignement des textures communes dans la superposition finale.
  3. Désactivez RANSAC en augmentant le seuil de tolérance à l'infini. Constatez l'effet de cisaillement provoqué par les outliers.

---

## 17.8 — L'état de l'art : la révolution de l'apprentissage profond

> *Des réseaux entraînés à détecter et à décrire*
...
## Encadré final — l'invariance est un tri
...
|---|---|---|---|
| `fig_ch17_couverture` | Chapeau | Deux photos d'une même façade avec des flèches reliant des points-clés homologues | JPG/PNG |
| `fig_ch17_01_ssd_vs_descripteur` | 17.1 | SSD sur patchs bruts (sensible à la rotation/lumière) vs descripteur invariant | SVG |
| `fig_ch17_02_echelle_caracteristique` | 17.2 | Espace d'échelle (gaussiennes de sigma croissants) et extrema du LoG | SVG |
| `fig_ch17_03_hog_glyphes` | 17.3 | Visualisation HOG (glyphes d'orientations) sous rotation et lumière | SVG |
| `fig_ch17_04_sift_orientation` | 17.4 | Orientation dominante tournant la carte locale du descripteur | SVG |
| `fig_ch17_05_brief_binaire` | 17.5 | BRIEF binaire : comparaisons de paires de pixels | SVG |
| `fig_ch17_06_ratio_test` | 17.6 | Distribution des distances d'appariement et seuil de Lowe | SVG |
| `fig_ch17_07_ransac` | 17.7 | Consensus RANSAC : tirages de points, inliers verts vs outliers rouges | SVG |
| `fig_ch17_08_loftr` | 17.8 | Appariement dense appris par LoFTR sur surfaces peu texturées | SVG |
