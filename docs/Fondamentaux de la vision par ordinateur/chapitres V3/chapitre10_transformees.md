# Chapitre 10 — Changer de regard : les transformées

![Un accordeur décompose une note complexe en ses fréquences pures ; en arrière-plan, la même image vue dans plusieurs systèmes de coordonnées](../figures/fig_ch10_couverture.jpg)
*Une transformée ne change pas l'image, seulement le système de coordonnées où on la lit. Le bon repère rend trivial ce qui semblait laborieux : effacer une rayure devient effacer deux coefficients.*
...
Le chapitre prolonge le chapitre 5 : il révèle *pourquoi* les filtres spatiaux agissent sur les fréquences. La transformée de distance (§10.5) reparaîtra au chapitre 12 sur la segmentation.
...
## 10.1 — La transformée de Fourier (DFT) : décomposer en ondes

> *L'accordeur qui décompose une note en ses fréquences pures*

![fig_ch10_obs1_fft_filter](../figures/fig_ch10_obs1_fft_filter.svg)
...
### Piège — l'image est supposée se répéter
...
## 10.2 — Le théorème de convolution : le pont avec le chapitre 5

> *Faire glisser un noyau, ou multiplier deux spectres — le même geste*
...
> *Replier le tissu comme un miroir pour éviter la couture visible*
...
## 10.4 — La transformée de Hough : voter pour des formes

> *Un vote à bulletin secret : les vrais alignements s'accumulent, le bruit se disperse*

![fig_ch10_obs3_hough](../figures/fig_ch10_obs3_hough.svg)
...
### La formule — la dualité point/courbe
...
## 10.5 — La transformée de distance : mesurer une forme de l'intérieur

> *Du masque présence/absence à une carte de relief*

![fig_ch10_obs2_distance_transform](../figures/fig_ch10_obs2_distance_transform.svg)

### L'intention

Une forme binaire ne dit que « appartient / n'appartient pas ». On voudrait savoir, pour chaque pixel intérieur, *à quelle profondeur* il appartient — pour en extraire l'axe, l'épaisseur, le point le plus intérieur.
...
## Encadré final — reconnaître la base où la question devient facile
...
|---|---|---|---|
| `fig_ch10_couverture` | chapeau | Illustration : accordeur décomposant une note ; même scène vue en plusieurs repères | JPG/PNG |
| `fig_ch10_obs1_fft_filter` | 10.1 | Déjà existant (SVG) : spectre FFT, deux pics de rayures, filtrage | — |
| `fig_ch10_obs3_hough` | 10.4 | Déjà existant (SVG) : point→courbe, droite→pic dans l'accumulateur | — |
| `fig_ch10_obs2_distance_transform` | 10.5 | Déjà existant (SVG) : masque → carte de relief, crêtes = squelette | — |
| `fig_ch10_01_dct_jpeg` | 10.3 | Bloc 8×8 ciel (1 nombre) vs feuillage (énergie étalée) | SVG |
| `fig_ch10_02_conv_theorem` | 10.2 | Schéma : glisser un pochoir ⇔ multiplier deux spectres | SVG |
