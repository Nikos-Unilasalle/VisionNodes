...
#subtitle[Si l'intensité était une altitude, l'image deviendrait un paysage. Le gradient pointe la plus forte pente, les contours sont ses falaises — et le moindre caillou de bruit s'y fait passer pour un sommet.]
...
== Le gradient d'image : la boussole de la pente

#subtitle[Une flèche qui pointe vers la plus forte montée d'intensité]

#figfull("/illustrations/chap6.1.jpeg")
...
#warning-box(title: "Piège — la bonne arc-tangente")[
...
== Sobel et Scharr : dériver en lissant de travers

#subtitle[Mesurer la pente sur trois lignes à la fois, pas une seule]
...
#warning-box(title: "Piège — garder les valeurs négatives")[
...
== Le détecteur de Canny : un pipeline qui ose le compromis
...
L'étape clé est la *suppression non-maximale*. Le gradient brut donne des contours épais, car la transition s'étale sur plusieurs colonnes. On ne garde un pixel que s'il est le plus fort de ses voisins _dans la direction du gradient_ : on regarde les deux voisins de part et d'autre, et si le pixel central n'est pas le plus intense des trois, on le supprime. Le contour est ainsi réduit à sa crête, large d'un pixel — comme on ne garde que le point culminant d'une pente.
...
#info-box(title: "Réglage — trois paramètres liés")[
...
== Le tenseur de structure : plat, bord ou coin ?

#subtitle[Le détective qui observe toute la zone, pas une seule empreinte]
...
== Coins : Harris et Shi-Tomasi

#subtitle[Reconnaître un coin sans calculer ses valeurs propres une à une]
...
=== La forme recherchée
Deux raccourcis d'algèbre donnent les valeurs propres _indirectement_, par deux quantités faciles à calculer à partir du tenseur de structure : leur *produit* et leur *somme*.
