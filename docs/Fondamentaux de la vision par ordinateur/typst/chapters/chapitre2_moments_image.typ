...
#subtitle[Découpez la silhouette dans du carton et toute la mécanique du solide s'applique : où elle tient en équilibre, autour de quel axe elle tourne le plus facilement. Les moments d'image ne mesurent rien d'autre.]
...
== Moments bruts : poser des questions à la forme

#subtitle[Peser chaque pixel selon l'endroit où il se trouve]
...
== Centroïde : le point d'équilibre

#subtitle[L'endroit où la plaque de carton tient sur une pointe]
...
== Moments normalisés : oublier la taille

#subtitle[Deux photos d'un même objet, l'une de près, l'autre de loin]
...
+ Son aire (le moment d'ordre 0, `μ₀₀`) grandit comme une surface, c'est-à-dire proportionnellement à `L²`.
+ Un moment d'ordre `p + q` (comme `μ_pq`) somme les coordonnées des pixels élevées aux puissances `p` et `q`. Sa valeur totale grandit donc à la fois selon l'aire (`L²`) et selon la distance des pixels au centre (`L^(p+q)`). Sa valeur croît donc proportionnellement à `L^(p + q + 2)`.
+ Pour annuler l'effet de la taille L dans le rapport `μ_pq / μ₀₀^γ`, nous devons faire en sorte que le haut et le bas grandissent à la même vitesse. Nous voulons donc que `L^(p + q + 2)` soit égal à `(L²)^γ`, ce qui s'écrit `L^(p + q + 2) = L^(2γ)`.
+ Pour que cela fonctionne quelle que soit la taille L, les exposants doivent être égaux : `p + q + 2 = 2γ`.
+ En divisant par 2, on trouve le fameux exposant : `γ = (p + q)/2 + 1`.
...
#info-box(title: "Limite — exacte en théorie, approchée en pratique")[
...
== Orientation principale : l'axe de moindre effort

#subtitle[La règle plate pivote sans peine autour de son grand axe]
...
#info-box(title: "Subtilité — l'orientation d'un objet rond n'existe pas")[
...
== Ellipse équivalente : le résumé d'une silhouette

#subtitle[L'ellipse qui pèse comme la forme, sans la mesurer]
...
#warning-box(title: "Piège — l'ellipse ne mesure pas la taille de l'objet")[
...
== Les sept invariants de Hu : la signature d'une forme

#subtitle[Reconnaître une chanson quelle que soit la tonalité]
...
== Moments pondérés par l'intensité

#subtitle[Rendre les pixels clairs plus lourds que les sombres]
