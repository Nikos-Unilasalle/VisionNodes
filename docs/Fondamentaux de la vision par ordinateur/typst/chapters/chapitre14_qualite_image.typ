...
+ L'électronique du capteur : la lecture du signal ajoute une perturbation fixe, présente même dans l'obscurité complète. La variance de ce bruit de lecture est constante, indépendante de la lumière.
...
```
_Schéma à produire ; script de référence en Annexe 1, §A1‑14.1._
...
== MSE et PSNR : l'observateur aveugle à la structure
...
Le cas (b) est la démonstration canonique : une image visuellement _inchangée_ — juste décalée d'un pixel — reçoit le pire score possible, parce que l'observateur-MSE compare des positions absolues. Pixel par pixel, tout a changé ; structurellement, rien.
]

#info-box(title: "Subtilité — calculer en virgule, en espace linéaire, et fixer MAX")[
...
#info-box(title: "Différence d'implémentation — plage, fenêtre, couleur")[
...
== Entropie d'image : la qualité sans référence

#subtitle[Combien de bits pour noter la valeur d'un pixel tiré au hasard]
...
#info-box(title: "Subtilité — base du log, binning, et jamais seule")[
...
== Métriques perceptuelles apprises (LPIPS) : l'observateur ajusté sur l'humain

#subtitle[Comparer dans l'espace où un réseau a appris à voir comme nous]

=== L'intention
Les métriques précédentes échouent sur un cas crucial : une texture _inventée_ mais plausible (super-résolution, génératif) que l'humain préfère, mais que le pixel condamne. On veut une mesure calée sur la *préférence humaine réelle*, pas sur une coïncidence de pixels ni un modèle artisanal.
...
=== Ce qu'elle mesure, et son angle mort
L'observateur est un réseau calibré sur la préférence humaine ; LPIPS suit la perception bien mieux que PSNR/SSIM sur de nombreuses distorsions, en particulier les artefacts des modèles génératifs. Quatre angles morts. Il hérite des *biais de son réseau de base* et peut se tromper sur du contenu hors distribution (imagerie médicale, télédétection). Il reste _full-reference_, lourd (un modèle à charger), et *non interprétable* — on ignore _pourquoi_ deux images sont jugées dissemblables. C'est encore un _modèle_ de l'observateur, pas l'observateur : l'optimiser directement comme coût crée des artefacts qui flattent la métrique sans plaire à l'œil. Pour le sans-référence, d'autres existent — NIQE et BRISQUE (statistiques), MUSIQ et MANIQA (appris).

#question-box(title: "Exemple — le compromis perception/distorsion")[
