...
#chapter(title: [Mesurer un écart : distances et similarités], toc: false)[

#figtodo("chap3", [Illustration de couverture du chapitre 3])
...
== Distances de Minkowski (L_p) : un seul bouton, trois distances

#subtitle[Du taxi en damier au vol d'oiseau, selon comment on agrège les écarts]
...
=== La forme recherchée
Le réglage est un exposant, noté p, qui décide du poids relatif des grands et des petits écarts. À p = 1, on additionne les écarts tels quels, comme un taxi parcourant des rues en damier (d'où le nom _Manhattan_). À p = 2, on prend la diagonale directe, le vol d'oiseau — la distance ordinaire. Plus p grandit, plus élever à cette puissance écrase les petits écarts devant le plus grand, jusqu'à ce que *seul le plus grand écart compte encore* : c'est la limite L∞, le simple maximum.
...
- p = 1 : distance de Manhattan (L1, _city-block_)
...
#info-box(title: "Limite — la malédiction de la dimension")[
...
== Distance de Mahalanobis : compter en variations typiques
...
#warning-box(title: "Piège — une covariance mal estimée")[
...
== Similarité cosinus : la direction, pas la longueur

#subtitle[Un long article et son résumé pointent dans le même sens]
...
+ *L'image de l'ombre portée* : Imaginez qu'une lampe est placée verticalement au-dessus du deuxième vecteur. Le produit scalaire `x · y` mesure la longueur de « l'ombre » projetée par le premier vecteur sur le second, multipliée par la longueur de ce second vecteur. C'est une mesure directe de leur complicité géométrique.
+ *Le calcul pas à pas* : Pratiquement, on multiplie les coordonnées des deux vecteurs face à face, puis on additionne tout : `x₁·y₁ + x₂·y₂ + ...`. Si les deux vecteurs ont des composantes fortes aux mêmes endroits, la somme explose ; s'ils s'évitent (l'un a des valeurs là où l'autre a des zéros), le produit scalaire tombe à zéro.
+ *Le rôle du dénominateur* : Puisque cette ombre portée dépend de la taille des flèches, diviser par le produit de leurs longueurs `‖x‖ · ‖y‖` permet d'annuler cet effet d'échelle. Il ne reste que la pure direction : le cosinus de l'angle `θ` formé par les deux flèches.
...
== Distances entre histogrammes : pondérer les cases

#subtitle[On s'étonne plus d'un écart sur un événement rare que sur un banal]
...
#info-box(title: "Limite — la sensibilité au découpage en cases")[
...
== Distance de transport (Wasserstein / EMD) : le coût du déménagement
...
=== La forme recherchée
L'image fondatrice est celle de *tas de sable* que l'on veut remodeler. Combien de travail pour transformer un tas en un autre, sachant qu'on paie distance × quantité déplacée ? La _Earth Mover's Distance_ — la distance du « déplaceur de terre » — mesure ce *coût minimal de transport*. Deux histogrammes décalés d'une seule case ont une EMD petite (le coût d'un petit déplacement), là où la χ² les déclarait maximalement différents. L'EMD respecte la proximité perceptuelle que les mesures case-à-case ignorent.
...
#info-box(title: "La formule — le cas 1-D, sans optimisation")[
```
...
#info-box(title: "Limite — le coût de calcul au-delà de 1-D")[
...
== Distance de Hausdorff : le pire écart de frontière

#subtitle[La pire excursion d'un point hors de l'autre ensemble]
...
#info-box(title: "Sensibilité — un seul point aberrant suffit")[
