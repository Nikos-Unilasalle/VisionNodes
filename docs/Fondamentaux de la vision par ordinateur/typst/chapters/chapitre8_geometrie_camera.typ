...
#subtitle[Capturer le monde en image, c'est l'aligner de force sur une surface plate. Quelque chose se perd dans l'opération. Ce chapitre explique quoi, pourquoi, et comment le récupérer.]
...
== Coordonnées homogènes : linéariser la projection

#subtitle[Reporter une retenue, pour calculer proprement et ne diviser qu'à la fin]
...
Ce formalisme représente les _points à l'infini_, impossibles à écrire en coordonnées ordinaires. Deux rails parallèles convergent en image vers un point de fuite, qui s'écrit simplement `(x, y, 0)` en homogène (un W nul, donc une division par zéro escamotée). Ce point se calcule, s'extrait, s'utilise — pour détecter l'horizon, estimer la géométrie d'une route, redresser une image prise en biais.

#info-box(title: "Subtilité — toujours diviser par W avant de lire")[
...
== Le modèle sténopé : du monde réel au pixel

#subtitle[L'ombre portée d'un point ne dit plus à quelle distance il était]

=== L'intention
En contrôle qualité industriel, on veut mesurer en millimètres la taille d'un défaut visible dans l'image. Convertir des pixels en millimètres suppose de connaître exactement comment la caméra a transformé la scène en image. Ce modèle de la transformation s'appelle le *modèle sténopé* (_pinhole_).
...
== Calibration : apprendre à sa caméra ses propres règles

#subtitle[Sans calibration, un pixel ne mesure rien]
...
#info-box(title: "Réglage — diversifier les poses")[
...
== L'homographie : corriger la perspective d'une surface plane

#subtitle[Redresser le regard oblique de la caméra sur une table, un sol, une affiche]

#figfull("/illustrations/chap8.4.png")
...
+ *Une scène plane* : Tous les objets photographiés résident sur un même plan physique (comme une affiche au mur, une table, ou un sol d'entrepôt plat). S'il y a du relief (ex. : des boîtes en volume posées sur le sol), l'homographie va les coucher et les étirer artificiellement.
+ *Une rotation pure de la caméra* : La caméra pivote sur son propre centre sans aucun déplacement dans l'espace (comme lorsqu'on prend un panorama). Dans ce cas, même si la scène a du relief, la perspective ne change pas d'une vue à l'autre et l'homographie reste exacte.
...
== La géométrie épipolaire : chercher en ligne, pas en surface
...
#info-box(title: "Différence d'implémentation — la normalisation de Hartley")[
...
== Stéréovision : retrouver la profondeur perdue

#subtitle[Le décalage entre deux vues mesure la distance, comme deux yeux jugent le relief]

#figfull("/illustrations/chap8.6.png")
...
`f` : focale en pixels. `B` : distance entre les deux caméras (_baseline_). `d` : disparité, le décalage horizontal en pixels du même point entre image gauche et droite. La relation inverse `Z ∝ 1/d` a trois conséquences à connaître :
...
Le vrai défi est de *mesurer la disparité* : trouver pour chaque pixel gauche son homologue exact à droite. La rectification (alignement des lignes épipolaires à l'horizontale) ramène la recherche sur une seule ligne. Les ennemis : zones sans texture (ciel, murs unis), occlusions, reflets spéculaires. L'algorithme SGBM (_Semi-Global Matching_) régularise la carte de disparité en imposant une cohérence spatiale entre pixels voisins.
]

#info-box(title: "Différence d'implémentation — la disparité en 1/16 de pixel")[
...
== Distorsion : corriger ce que l'objectif déforme
...
`(x, y)` désignent les coordonnées _idéales_ (sans distorsion), `r = √(x²+y²)` la distance au centre. Les coordonnées réellement mesurées valent :
