...
class DisplayOutput(NodeProcessor):
    def process(self, inputs, params):
        ...
        # Collect dynamic image inputs — all non-reserved keys (engine strips color prefix,
        # so actual keys are like '0_abc123', not 'image_0_abc123')
        ...
        if not images:
            ...
            if len(images) == 1:
                ...
                else: # Fallback to Side by Side for other modes if N > 2
                    h_max = max(img.shape[0] for img in images)
        ...
        if mask is not None:
            ...
            overlay[m_bin > 0] = [0, 0, 255] # Red BGR
