...
@vision_node(
    ...
    params=[
        {'id': 'padding', 'label': 'Padding (%)', 'type': 'int', 'min': 0, 'max': 100, 'default': 10}
    ]
)
class CropperNode(NodeProcessor):
    def process(self, inputs, params):
        ...
        if img is None or data is None: return {"main": img}

        # Accept list (face/hand detection) or single dict
        if isinstance(data, list):
            data = data[0] if data else None
        if not isinstance(data, dict) or 'xmin' not in data:
            return {"main": img}
