...
@vision_node(
    ...
    icon='FileText',
    description=(
        'Generates a standardized hematology report from cell counts.\n\n'
        'Connect the "count" output of an Object Extractor node to each input.\n\n'
        'Inputs:\n'
        '1. RBC count\n'
        '2. Neutrophils count\n'
        '3. Lymphocytes count\n'
        '4. Monocytes count\n'
        '5. Platelets count\n\n'
        'Calculates WBC differential percentages and provides biological interpretation.'
    ),
    inputs=[
        {'id': 'count_rbc', 'label': 'RBC Count',          'color': 'scalar'},
        {'id': 'count_neu', 'label': 'Neutrophils Count',   'color': 'scalar'},
        {'id': 'count_lym', 'label': 'Lymphocytes Count',   'color': 'scalar'},
        {'id': 'count_mon', 'label': 'Monocytes Count',     'color': 'scalar'},
        {'id': 'count_plt', 'label': 'Platelets Count',     'color': 'scalar'},
...
class HemogrammeNode(NodeProcessor):
    def process(self, inputs, params):
        rbc_count = int(inputs.get('count_rbc') or 0)
        neu_count = int(inputs.get('count_neu') or 0)
        lym_count = int(inputs.get('count_lym') or 0)
        mon_count = int(inputs.get('count_mon') or 0)
        plt_count = int(inputs.get('count_plt') or 0)

        wbc_total = neu_count + lym_count + mon_count

        calib   = inputs.get('calibration', 1.0) or 1.0
        ...
        def draw_row(label, val, unit_str="", color=(40, 40, 40)):
            ...
            cv2.putText(img, f"{val} {unit_str}", (220, y),
        ...
        for i, line in enumerate(comment.split('\n')):
            if i > 5:
                break
            cv2.putText(img, line, (25, y + 25 + i * 20),
