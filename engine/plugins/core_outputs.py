    ...
    def process(self, inputs, params):
        ...
        def to_bgr(img):
            if not isinstance(img, np.ndarray):
                # Scalar or non-array: render as text image
                canvas = np.zeros((80, 320, 3), dtype=np.uint8)
                cv2.putText(canvas, str(img)[:40], (8, 48), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (200, 200, 200), 2)
                return canvas
        ...
        # No image inputs: fall back to flow_in / mask_in (may be scalar)
        if not images:
            fallback = flow if flow is not None else mask
            if fallback is None:
                return {"main": None}
            res = to_bgr(fallback)
        elif len(images) == 1:
            ...
            if mode == 0:  # Side by Side
                ...
                if len(images) > 1 and ph > 0 and pw > 0:
                    ...
                    res[h-ph-10:h-10, w-pw-10:w-10] = pip_img
